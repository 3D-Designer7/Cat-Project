...
async function test() {
  ...
  const { data: d1, error: e1 } = await supabase.auth.signUp({ email, password: '1' });
  console.log('signUp non-existent:', { data: d1, error: e1 });

  console.log('Checking existing email...');
  const { data: d2, error: e2 } = await supabase.auth.signUp({ email: 'hamidra7a@gmail.com', password: '1' });
